...
const Form = React.forwardRef((props, ref) => {
  ...
  const handleCreate = async (document) => {
    if (name !== "" && image !== null) {
      let response = await addDocument("clients", document);
      if (response) {
        setIsLoading(false);
        Swal.fire({
  ...
  const handleUpdate = async (document, id) => {
    let response = await updateDocument("clients", document, id);
    if (response) {
      setIsLoading(false);
      Swal.fire({
  ...
  }, [props.isEdit, props.document.name, props.document.imagePath]);
